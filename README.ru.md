...
- PHP 8.3 – 8.5
- `ext-mbstring`
- `rasuvaeff/property-testing-core` `^0.9 || ^0.10 || ^0.11 || ^0.12`
...
`$last`, `$gender` и тремя формами отображения. Он `Stringable` — `(string) $p`
даёт `full()` — и `json_encode()` рендерит его как обычные данные, потому что
`Gender` — enum со строковым значением (`'male'`, `'female'`). Конструктор
отвергает пустую или состоящую из одних пробелов часть с
`InvalidArgumentException`, в котором названа часть (`First name must not be
empty`); «нет отчества» — это `$middle === null`, а не `''`.
...
### Объекты, хранящие имя

`Gen::forClass(PersonName::class)` не использует наборы данных: он читает
аннотации `non-empty-string` конструктора и собирает имена из произвольного
текста — в том числе из строк из одних пробелов, которые конструктор отвергает.
Когда тестируемый класс хранит `PersonName`, передайте `Names::person()` как
override для этого параметра, чтобы значение приходило из списков и
шринкалось к обычному имени:
...
У `Gender` два случая, `Male` (`'male'`) и `Female` (`'female'`), объявленные
именно в этом порядке: шринкинг идёт к первому случаю.
...
Для регрессионного корпуса это существенно. Контрпример, аргумент которого —
`PersonName`, хранится как *seed*-запись: у кодека ядра нет представления
данными для объектов, кроме enum, — и минор с изменением набора заставляет
этот seed воспроизвести другого человека, то есть записанное падение молча
перестаёт быть тем, что было найдено. Контрпример из строк (`Names::full()`,
`first()`, `last()`) хранится как *values* и воспроизводится через любой
релиз; если корпус должен переживать апгрейды, пишите свойство над ними. По
той же причине `CounterExample::toExamplesCode()` бросает исключение на
аргументе `PersonName`.
